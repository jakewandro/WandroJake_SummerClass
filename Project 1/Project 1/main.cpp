...
int main(int argc, char** argv) {
    ...
    int pclass, random, flee, bdmg, ddmg, choice;
    ...
    cout<<"2) no"<<endl;
    cin>>tip;
    if(tip=='1') cout<<"Tip: Don't die"<<endl;
    //Process the Data
    ...
    do{
        cout<<name<<": "<<hp<<"        Bandit: "<<bandit<<endl; //Display health
    ...
    }while(hp>=1&&bandit>=1);  //Continue while they are both alive
    if(hp<=0) cout<<"You have died"<<endl; //If you die you lose
    else{
    ...
    if(hp<=0) cout<<"You have died"<<endl; //Lose
    else{                                  //Win
